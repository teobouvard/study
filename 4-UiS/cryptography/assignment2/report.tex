...
The implementation of a symmetric cipher is not part of this assignment as it has already been done in Assignment 1. We chose AES for its ingenuity and subsequent popularity. We use the Electronic Code Book mode of operation so that we do not have to transfer initialization vectors or nonces with the message. In a real-world application, another mode would be used as ECB is semantically insecure as a given plaintext is encrypted to the same ciphertext each time. For short text messages, this mode of operation is probably reasonable enough.
...
\begin{figure}[h]
    ...
    \caption{Bitmap representation of generated PRN}
