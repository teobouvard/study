...
// For further information, see the PPM image format documentation at
...
const char *wbLogLevelToStr(const wbLogLevel level) {
  wbAssert(level >= wbLogLevel_OFF && level < wbLogLevel_INVALID,
           "Unrecognized wbLogLevel value");
...
template <typename T1> inline void wbLog(T1 const &p1) { std::cout << p1; }
...
inline void wbLog(T1 const &p1, T2 const &p2, T3 const &p3, T4 const &p4,
                  T5 const &p5, T6 const &p6, T7 const &p7) {
...
inline void wbLog(T1 const &p1, T2 const &p2, T3 const &p3, T4 const &p4,
                  T5 const &p5, T6 const &p6, T7 const &p7, T8 const &p8) {
...
char *wbArg_getInputFile(const wbArg_t argInfo, const int argNum) {
  wbAssert(argNum >= 0 && argNum < (argInfo.argc - 1),
           "Unrecognized command line argument requested");
...
float *wbImport(const char *fName, int *numElements) {
  ...
  if (!fBuf) {
    std::cerr << "Unable to allocate memory for an array of size "
              << *numElements * sizeof(float) << " bytes" << std::endl;
    inFile.close();
    std::exit(EXIT_FAILURE);
  }
...
int *wbImport(const char *fName, int *numElements, std::string type) {
  ...
  if (!fBuf) {
    std::cerr << "Unable to allocate memory for an array of size "
              << *numElements * sizeof(int) << " bytes" << std::endl;
    inFile.close();
    std::exit(EXIT_FAILURE);
  }
...
float *wbParseCSV(const char *fName, int *numRows, int *numCols) {
  ...
  *numRows = *numCols = 0;
    ...
    *numCols = 0;
    ...
    while (std::getline(rowStream, cellStr, ',')) {
      ...
      if (!(std::istringstream(cellStr) >> fVal)) {
        std::cerr << "Error reading element (" << *numRows << ", " << *numCols
  ...
  if ((*numRows != *numCols) || (0 == numElements)) {
    std::cerr << "Error reading contents of file " << fName
              << ". Last element read (" << *numRows << ", " << *numCols << ")"
              << std::endl;
    std::exit(EXIT_FAILURE);
  }
...
struct wbImage_t {
  ...
  wbImage_t(int imageWidth = 0, int imageHeight = 0,
      ...
      : width(imageWidth), height(imageHeight), channels(imageChannels),
        colors(imageColors), data(NULL) {
...
wbImage_t wbImport(const char *fName) {
  ...
  if (!inFile.is_open()) {
    std::cerr << "Error opening image file " << fName << ". "
              << wbInternal::wbStrerror(errno) << std::endl;
    std::exit(EXIT_FAILURE);
  }
  ...
  if (magic != "P6") {
    std::cerr << "Error reading image file " << fName << ". "
              << "Expecting 'P6' image format but got '" << magic << "'"
              << std::endl;
    inFile.close();
    std::exit(EXIT_FAILURE);
  }
  ...
  if (inFile.fail() || image.colors != wbInternal::kImageColorLimit) {
    std::cerr << "Error reading colors value of image in file " << fName
              << std::endl;
    inFile.close();
    std::exit(EXIT_FAILURE);
  }
...
int wbImage_getWidth(const wbImage_t &image) { return image.width; }

int wbImage_getHeight(const wbImage_t &image) { return image.height; }

int wbImage_getChannels(const wbImage_t &image) { return image.channels; }

float *wbImage_getData(const wbImage_t &image) { return image.data; }

wbImage_t wbImage_new(const int imageWidth, const int imageHeight,
                      const int imageChannels) {
  wbImage_t image(imageWidth, imageHeight, imageChannels);
  return image;
}

void wbImage_delete(wbImage_t &image) { delete[] image.data; }
  ...
  double value() {
    ...
    if (0 == tb.denom)
      (void)mach_timebase_info(
          &tb); // Calculate ratio of mach_absolute_time ticks to nanoseconds
...
public:
  ...
  double value() { return ((double)endTime - startTime) / NSEC_PER_SEC; }
};
#endif
} // namespace wbInternal
...
void wbTime_start(const wbTimeType timeType, const std::string timeMessage) {
  wbAssert(timeType >= Generic && timeType < wbTimeTypeINVALID,
           "Unrecognized wbTimeType value");
...
void wbTime_stop(const wbTimeType timeType, const std::string timeMessage) {
  ...
  const wbInternal::wbTimerInfo searchInfo = {timeType, timeMessage,
                                              wbInternal::wbTimer()};
  const wbInternal::wbTimerInfoList::iterator iter =
      std::find(wbInternal::timerInfoList.begin(),
                wbInternal::timerInfoList.end(), searchInfo);

  wbInternal::wbTimerInfo &timerInfo = *iter;

  wbAssert(searchInfo == timerInfo,
           "Could not find a corresponding wbTimerInfo struct registered by "
           "wbTime_start()");
...
bool wbFPCloseEnough(const float u, const float v) {
  // Note that the tolerance level, e, is still an arbitrarily chosen value.
  // Ideally, this value should scale std::numeric_limits<float>::epsilon() by
  // the number of rounding operations
  const float e = 0.0005f;

  // For floating point values u and v with tolerance e:
  //   |u - v| / |u| <= e || |u - v| / |v| <= e
  // defines a 'close enough' relationship between u and v that scales for
  // magnitude See Knuth, Seminumerical Algorithms 3e, s. 4.2.4, pp. 213-225
  return ((fabs(u - v) / fabs(u == 0.0f ? 1.0f : u) <= e) ||
          (fabs(u - v) / fabs(v == 0.0f ? 1.0f : v) <= e));
}
} // namespace wbInternal
...
void wbSolution(const wbArg_t &args, const T &t, const S &s, const U &u) {
  ...
  float *soln = wbImport(wbArg_getInputFile(args, args.argc - 2), &solnRows,
                         &solnColumns);

  if (solnRows != s || solnColumns != u) {
    std::cout << "Size of the matrix in solution file "
              << wbArg_getInputFile(args, args.argc - 2) << " does not match. ";
    std::cout << "Expecting " << solnRows << " x " << solnColumns << " but got "
              << s << " x " << u << ".\n";
  } else // Check solution
  {
    int errCnt = 0;
...
namespace wbInternal {
// For assignments MP6 & MP11
void wbImage_save(const wbImage_t &image, const wbArg_t &args,
                  const char *fName) {
  std::ostringstream oss;
  oss << "P6\n"
      << "# Created by applying convolution "
      << wbArg_getInputFile(args, args.argc - 3) << "\n"
  ...
  for (int i = 0; i < numElements; ++i) {
    rawData[i] = static_cast<unsigned char>(
        image.data[i] * wbInternal::kImageColorLimit + 0.5f);
  }
...
void wbSolution(const wbArg_t &args, const wbImage_t &image) {
  ...
  if (solnImage.width != image.width || solnImage.height != image.height) {
    std::cout << "Size of the image in file "
              << wbArg_getInputFile(args, args.argc - 2) << " does not match. ";
      ...
      for (int j = 0; j < image.height; ++j) {
        for (int k = 0; k < image.channels; ++k) {
          ...
          if (error > (1.0f / wbInternal::kImageColorLimit * tolerance)) {
            if (errCnt < wbInternal::kErrorReportLimit)
              std::cout << "Image pixels do not match at position (" << j
                        << ", " << i << ", " << k << "). [" << image.data[index]
                        << ", " << solnImage.data[index] << "]\n";
